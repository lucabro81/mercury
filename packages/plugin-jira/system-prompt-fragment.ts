 ...
 * irreversible deletes, and the DON'Ts. The core's `buildSystemPrompt` inserts
 * this verbatim among the fragments of whatever plugins loaded, only when this
 * plugin is active — the same position and text the inline `if (opts.jira)`
 * block held before the extraction. Kept as its own leaf module so both the
 * plugin object (`plugin.ts`) and the package surface (`index.ts`) can import
 * it without a cycle.
 */
