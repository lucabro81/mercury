 ...
 * `messages send` commands. Not a model-invocable tool — reading/sending
 * Google Chat messages is the transport of the channel itself (see
 * `src/router/channels/google-chat-events.ts`), the same way stdin/
 * stdout is the transport for the terminal channel, never something the
 * model chooses to invoke like `jiraCli`.
 *
