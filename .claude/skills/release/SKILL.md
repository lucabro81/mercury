...
**2. Get the actual change committed first.** `scripts/tag-release.sh` only stages `apps/mercury/package.json`, `apps/mercury/CHANGELOG.md`, and `.changeset` — it does *not* pick up unrelated working-tree changes. If the work being released isn't committed yet, commit it now (small atomic commits per logical unit, same as any other commit in this repo). Only after that do you write the changeset — the changeset describes committed history, not a promise of what's still sitting in the working tree.
...
**6. Release it.** Run `bun run release` from the repo root. This bumps `apps/mercury/package.json`, writes the `apps/mercury/CHANGELOG.md` entry, commits both plus `.changeset`, and creates a local `git tag vX.Y.Z` — all expected, agreed-on behavior of this command. Don't caveat or ask before it runs; that conversation already happened when this workflow was set up. It never pushes anything.
